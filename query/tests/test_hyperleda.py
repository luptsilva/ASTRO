...
from .. import hyperleda, HyperLEDAClass
from .. import conf
...
def patch_request(request):
    ...
    mp.setattr(HyperLEDAClass, '_request',
               nonremote_request)
    return mp
...
def test_query_object(patch_request):
    result = HyperLEDAClass().query_object('UGC12591',
                                                   properties=['bt', 'vt'])
...
def test_query_sql(patch_request):
    ...
    result = HyperLEDAClass().query_sql(sample_query,
                                                       properties=['bt', 'vt'])
    assert isinstance(result, Table)
